...
{
    printf("I am in batchnorm weights\n");
...
void load_convolutional_weights_direct(layer l, FILE *fp)
{
    if (l.binary)
    {
        // load_convolutional_weights_binary(l, fp);
    ...
    if (l.numload)
        l.n = l.numload;
    //int num = l.c / l.groups * l.n * l.size * l.size;
    int num =  l.size * l.size * l.c / l.groups * l.n ;
    fread(l.biases, sizeof(float), l.n, fp);
    if (l.batch_normalize && (!l.dontloadscales))
    {
        fread(l.scales, sizeof(float), l.n, fp);
 ...
 for (int kh = 0; kh < l.size; kh++) // kernel height
    {
        ...
        {
            for (int oc = 0; oc < l.n; oc++) // output channel
            {
                for (int ic = 0; ic < l.c; ic++) // input channe;
                {
            
                    l.weights_hwcm[kh * l.size * l.c * l.n + kw * l.c * l.n + ic * l.n + oc] = (float)l.weights[oc * l.c * l.size * l.size + ic * l.size * l.size + kh * l.size + kw];
                }
            }
        }
    }
...
void load_weights_upto_direct(network *net, char *filename, int start, int cutoff)
{
    ...
    for (i = start; i < net->n && i < cutoff; ++i)
    {
        ...
        if (l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL)
        {
            load_convolutional_weights_direct(l, fp);
        }
        if (l.type == CONNECTED)
        {
            load_connected_weights(l, fp, transpose);
...
{
    load_weights_upto_direct(net, filename, 0, net->n);
}
