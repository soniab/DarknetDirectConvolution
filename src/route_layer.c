...
void forward_route_layer(const route_layer l, network net)
{
    int i, j;
    int offset = 0;
    printf("Route Layer = %d %d\n", l.n, l.batch);
    for(i = 0; i < l.n; ++i){
        int index = l.input_layers[i];
        float *input = net.layers[index].output;
        int input_size = l.input_sizes[i];
        printf("Value of input = %d offset = %d \n", input_size, offset);
        for(j = 0; j < l.batch; ++j){
    ...
    for (int oc = 0; oc < l.out_c; oc++) // output channel
    {
        for (int oh = 0; oh < l.out_h; oh++) // output height
        {
            for (int ow = 0; ow < l.out_w; ow++) // output width
            {

                printf("oc = %d oh = %d ow = %d %f\n", oc, oh, ow, l.output[oh * l.out_w * l.out_c + ow * l.out_c + oc]);
            }
        }
    }
}
