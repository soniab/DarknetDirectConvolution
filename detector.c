...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    //network *net = load_network(cfgfile, weightfile, 0);
    network *net = load_network_direct(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    srand(2222222);
    double time;
    char buff[256];
    ...
    while (1)
    {
        ...
        image im = load_image_color_direct(input, 0, 0);
        //image sized = letterbox_image(im, net->w, net->h);
        image sized = letterbox_image_direct(im, net->w, net->h);
        ...
        float *X = sized.data;
        //float *Transformed_x = transformed_func(sized, X, net->w, net->h);
        time = what_time_is_it_now();
        network_predict(net, X);
       // network_predict(net, Transformed_x);
